...
from transform.transformations import get_month_name, parse_date
...
def transform_times(db_con: Engine, etl_process_id: int) -> None:
    ...
    if not times_ext.empty:
        # Tranform columns
        df_times['TIME_ID'] = df_times['TIME_ID'].apply(parse_date)
        ...
        df_times['END_OF_CAL_MONTH'] = df_times['END_OF_CAL_MONTH'].apply(parse_date)
        df_times['CALENDAR_YEAR'] = df_times['CALENDAR_YEAR'].astype(int)
        df_times['CALENDAR_MONTH_NAME'] = df_times['CALENDAR_MONTH_INTEGER'].apply(get_month_name)
        # Add ETL process ID
